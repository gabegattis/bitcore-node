...
var WalletService = function(options) {
  ...
  this._MAX_QUEUE = 99999999999999;
  this._jobs = LRU({
    max: this._MAX_QUEUE,
      ...
      self._getAddresses(walletId, function(err, oldAddresses) {
        ...
        self._removeWallet(walletId, function(err) {
          ...
          self._createWallet(walletId, function() {
            ...
            self._importAddresses(walletId, oldAddresses, function(err) {
              if (err) {
...
WalletService.prototype._formatTransaction = function(tx) {
  ...
  for(var i = 0; i < tx.inputs.length; i++) {
    obj.inputs[i].satoshis = tx.__inputValues[i];
    obj.inputs[i].address = this.getAddressString(tx.inputs[i]);
  }

  for(var j = 0; j < tx.outputs.length; j++) {
    obj.outputs[j].address = this.getAddressString(tx.outputs[j]);
  }
